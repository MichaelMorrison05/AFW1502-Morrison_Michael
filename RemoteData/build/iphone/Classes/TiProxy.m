...
//Rare exceptions to indicate a bug in the _remotedata code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_remotedata = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_remotedata]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.remotedata.module"
