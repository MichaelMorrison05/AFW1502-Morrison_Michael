...
//Rare exceptions to indicate a bug in the _calendar code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_calendar = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_calendar]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.calendar.module"
