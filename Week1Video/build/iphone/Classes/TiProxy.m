...
//Rare exceptions to indicate a bug in the _week1video code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_week1video = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_week1video]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.week1video.module"
