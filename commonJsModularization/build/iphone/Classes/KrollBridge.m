...
NSString * commonJsModularization$ModuleRequireFormat = @"(function(exports){"
...
@implementation commonJsModularizationObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_commonjsmodularization);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create commonJsModularization global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "commonJsModularization" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_commonjsmodularization = [[commonJsModularizationObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_commonjsmodularization];
	
	NSString *_commonjsmodularizationNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _commonjsmodularizationNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_commonjsmodularization valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:commonJsModularization$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.commonjsmodularization.kroll" 
