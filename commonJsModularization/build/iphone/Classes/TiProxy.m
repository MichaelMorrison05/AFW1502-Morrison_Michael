...
//Rare exceptions to indicate a bug in the _commonjsmodularization code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_commonjsmodularization = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_commonjsmodularization]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.commonjsmodularization.module"
