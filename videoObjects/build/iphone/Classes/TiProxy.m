...
//Rare exceptions to indicate a bug in the _videoobjects code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_videoobjects = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_videoobjects]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.videoobjects.module"
